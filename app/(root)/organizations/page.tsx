...
async function getOrganizationsData() {
  return await serverService.organizations.findMany({});
}
      ...
      <div className="flex flex-1 flex-col p-4 md:p-6 gap-4 max-w-screen overflow-x-hidden">
        <div className="flex items-center justify-between">
          <div className="space-y-2">
            <h1 className="text-2xl font-bold tracking-tight md:text-3xl">Organizations Management</h1>
            <p className="text-sm text-muted-foreground">Manage your organizations data</p>
          </div>
        </div>
        
        <Suspense fallback={<Skeleton className="w-full h-96" />}>
          <OrganizationsView data={data} />
        </Suspense>
      </div>
    </>
  );
